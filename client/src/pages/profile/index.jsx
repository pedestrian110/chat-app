          ...
          onMouseEnter={() => setHovered(true)}
           ...
           >
            <Avatar className="h-32 w-32 md:w-48 rounded-full overflow-hidden">
             ...
             {<input type ="file" ref ={fileInputRef} className="hidden" onChange={handleImageChange} name ="profile-image" accept=".png, .jpg, .svg, .webp"></input>}
           </div>
            ...
            <div className="w-full flex gap-5">
               {
                ...
                ${selectedColor === index ? "outline outline-white/50 outline-1":""}`} key ={index} onClick={()=>setSelectedColor(index)}></div>)
               }
            </div>
           </div>
        </div>
        <div className="w-full">
          <Button className="h-16 w-full bg-purple-700 hover:bg-purple-900 transition-all duration-300" onClick={saveChangas}>Save Chages</Button>
        </div>
      </div>
    </div>
  )
